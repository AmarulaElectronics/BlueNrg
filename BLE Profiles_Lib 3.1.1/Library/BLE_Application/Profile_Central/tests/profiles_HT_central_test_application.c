...
/* Heart Rate Collector test configuration for some PTS tests */

///@cond BLE_CURRENT_PROFILE_ROLES & HEALTH_THERMOMETER_COLLECTOR
/**
 *@defgroup Health_Thermometer_Collector
 *@brief PTS test application user input commands for Health Thermometer Collector Central role
 *@{
 */
...
/** 
 *  Health Thermometer Collector user input command: Start Discovery procedure for detecting a Health Thermometer Sensor(<b>HT_Collector_DeviceDiscovery() </b> API)
 */
#define APPL_DISCOVER_HEALTH_THEMOMETER            'a' 

/** 
 *  Health Thermometer Collector user input command: Start Connection procedure for connecting to the discovered Health Thermometer Sensor(<b> HT_Collector_DeviceConnection() </b> API)
 */
#define APPL_CONNECT_HEALTH_THEMOMETER             'b' 

/** 
 *  Health Thermometer Collector user input command: Terminate the current connection with the Health Thermometer Sensor(<b> HT_Collector_DeviceDisconnection() </b> API)
 */
#define APPL_DISCONNECT_HEALTH_THEMOMETER          'c' 

/** 
 *   Health Thermometer Collector user input command: Start the pairing procedure (or enable the encryption) with the connected Health Thermometer Sensor (<b> HT_Collector_StartPairing() </b> API)
 */
#define APPL_PAIR_WITH_HEALTH_THEMOMETER           'd' 

/** 
 *  Health Thermometer Collector user input command: Clear the security database (it removes any bonded device) (<b>HT_Collector_Clear_Security_Database() </b> API)
 */
...
/** 
 *  Health Thermometer Collector user input command: Discover All Primary Services (<b>HT_Collector_ServicesDiscovery() </b> API)
 */
#define APPL_DISCOVER_HEALTH_THERMOMETER_SERVICES               'f' 

/** 
 *  Health Thermometer Collector user input command: Discover All Characteristics of Health Thermometer Services --> Temperature Measurement,Temperature Type, Intermediate Temperature, Measurement Interval characteristics (<b>HT_Collector_DiscCharacServ(HEALTH_THERMOMETER_SERVICE_UUID) </b> API)
 */
#define APPL_DISCOVER_TEMPERATURE_MEASUREMENT_CHAR              'g' 

/** 
 *  Health Thermometer Collector user input command: Discover Health Temperature Measurement Client Characteristic Configuration Descriptor (<b>HT_Collector_Start_Temperature_Measurement_Client_Char_Descriptor_Discovery() </b> API)
 */    
#define APPL_DISCOVER_TEMPERATURE_MEASUREMENT_CHAR_DESCRIPTOR   'h'

/** 
 *  Health Thermometer Collector user input command: Discover Health Intermediate Temperature Client Characteristic Configuration Descriptor (<b>HT_Collector_Start_Intermediate_Temperature_Client_Char_Descriptor_Discovery() </b> API)
 */  
#define APPL_DISCOVER_INTERMEDIATE_TEMPERATURE_CHAR_DESCRIPTOR  'k' 

/** 
 *  Health Thermometer Collector user input command: Discover Measurement Interval Client Characteristic Configuration Descriptor (<b>HT_Collector_Start_Measurement_Interval_Client_Char_Descriptor_Discovery() </b> API)
 */ 
#define APPL_DISCOVER_MEASUREMENT_INTERVAL_CHAR_DESCRIPTOR      'i' 

/** 
 *  Health Thermometer Collector user input command: Discover Device Information Service (DIS) Characteristics (<b>HT_Collector_DiscCharacServ(DEVICE_INFORMATION_SERVICE_UUID) </b> API)
 */
#define APPL_DISCOVER_DIS_CHARACTERISTICS                       'l' 

/** 
 *  Health Thermometer Collector user input command: Read Device Information Service Manufacturer Name Characteristic (<b>HT_Collector_ReadDISManufacturerNameChar() </b> API)
 */
#define APPL_READ_DIS_MANUFACTURER_NAME_CHAR                    'm' // TP/THS/CO/BV-14-I 

/** 
 *  Health Thermometer Collector user input command: Read Device Information Service Model Number Characteristic (<b>HT_Collector_ReadDISModelNumberChar() </b> API)
 */
#define APPL_READ_DIS_MODEL_NUMBER_CHAR                         'n' // TP/THS/CO/BV-14-I

/** 
 *  Health Thermometer Collector user input command: Read Device Information Service System ID Characteristic (<b>HT_Collector_ReadDISSystemIDChar() </b> API)
 */
#define APPL_READ_DIS_SYSTEM_ID_CHAR                            'o' // TP/THS/CO/BV-14-I 

//-------------------------- ENABLE NOTIFICATIONS and INDICATIONS

/** 
 *  Health Thermometer Collector user input command: Configure Health Temperature Measurement for Indication (<b>HT_Collector_Enable_Temperature_Measurement_Char_Indication() </b> API)
 */
#define APPL_ENABLE_TEMPERATURE_MEASUREMENT_CHAR_INDICATION     'p'

/** 
 *  Health Thermometer Collector user input command: Configure Health Intermediate Temperature for Notification (<b>HT_Collector_Enable_Intermediate_Temperature_Char_Notification() </b> API)
 */
#define APPL_ENABLE_INTERMEDIATE_TEMPERATURE_CHAR_NOTIFICATION  'q' 

/** 
 *  Health Thermometer Collector user input command: Configure Health Measurement Interval for Indication (<b>HT_Collector_Enable_Measurement_Interval_Char_Indication() </b> API)
 */
#define APPL_ENABLE_MEASUREMENT_INTERVAL_CHAR_INDICATION        'r' 
...
/** 
 *  Health Thermometer Collector user input command: Read Measurement Interval characteristic(<b>HT_Collector_Read_Measurement_Interval() </b> API)
 */
#define APPL_READ_MEASUREMENT_INTERVAL_CHAR               's'

/** 
 *  Health Thermometer Collector user input command: Write Measurement Interval characteristic(<b>HT_Collector_Write_Measurement_Interval() </b> API)
 */
#define APPL_WRITE_MEASUREMENT_INTERVAL_CHAR              't' 

/** 
 *  Health Thermometer Collector user input command: Read Measurement Interval Valid Range descriptor(<b>HT_Collector_Read_Measurement_Interval_Valid_Range_Descr() </b> API)
 */
#define APPL_READ_MEASUREMENT_INTERVAL_VALID_RANGE_DESC   'u' 

/** 
 *  Health Thermometer Collector user input command: Read Temperature Type characteristic(<b>HT_Collector_Read_Temperature_Type() </b> API)
 */
#define APPL_READ_TEMPERATURE_TYPE_CHAR                   'v'

/** 
 *  Health Thermometer Collector user input command: Starts all the connection, service discovery, characteristic discovery, ..... procedures.
 */
#define APPL_START_FULL_CONFIGURATION                     'w' 

/** 
 *  Health Thermometer Collector user input command: Display PTS user input commands list.
 */
...
/* HR collector discovery variables */
  ...
  for (i=0; i<numReporterServices; i++) {
    if (reporterServices[j] == UUID_TYPE_16) {
      ...
      case HEALTH_THERMOMETER_SERVICE_UUID:
        ...
        APPL_MESG_INFO(profiledbgfile,"(%c%c1)", PTS_CODE, HTP_PTS_CODE);
#endif         
	APPL_MESG_INFO(profiledbgfile,"****    Health Thermometer Service UUID found\r\n");
	break;
      case DEVICE_INFORMATION_SERVICE_UUID:
#ifdef PTS_AUTOMATING
        APPL_MESG_INFO(profiledbgfile,"(%c%c2)", PTS_CODE, HTP_PTS_CODE);
#endif  
...
void printDebugCharac(uint8_t numCharac, uint8_t *charac)
{
  ...
  for (i=0; i<numCharac; i++) { 
...
#ifdef PTS_AUTOMATING
      APPL_MESG_INFO(profiledbgfile,"(%c%c6)", PTS_CODE, HTP_PTS_CODE);
#endif       
...
#ifdef PTS_AUTOMATING
      APPL_MESG_INFO(profiledbgfile,"(%c%c7)", PTS_CODE, HTP_PTS_CODE);
#endif       
...
#ifdef PTS_AUTOMATING
      APPL_MESG_INFO(profiledbgfile,"(%c%c8)", PTS_CODE, HTP_PTS_CODE);
#endif       
    ...
    case SYSTEM_ID_UUID:
#ifdef PTS_AUTOMATING
      APPL_MESG_INFO(profiledbgfile,"(%c%c9)", PTS_CODE, HTP_PTS_CODE);
#endif       
...
#ifdef PTS_AUTOMATING
      APPL_MESG_INFO(profiledbgfile,"(%c%c10)", PTS_CODE, HTP_PTS_CODE);
#endif      
      APPL_MESG_INFO(profiledbgfile,"****      Client Characteristic Configuration Descriptor found\r\n");
      break;
    case VALID_RANGE_DESCRIPTOR_UUID:
#ifdef PTS_AUTOMATING
      APPL_MESG_INFO(profiledbgfile,"(%c%c11)", PTS_CODE, HTP_PTS_CODE);
#endif      
...
void Device_SetSecurity(void)
{
  ...
  param.sc_support = HTC_SC_SUPPORT;
  param.keypress_support = HTC_KEYPRESS_NOTIFICATION_SUPPORT;  
  param.identity_address_type = HTC_IDENTITY_ADDRESS_TYPE ;
#else  
...
void Host_Profile_Test_Application (void)
{
  ...
  APPL_MESG_INFO(profiledbgfile,"\n HTC Test Application: Starting testing the BLE Profiles (z for menu) \n" );
  ...
  {    
    ...
    switch(deviceState)
    {
      ...
      {
#ifdef PTS_AUTOMATING
        APPL_MESG_INFO(profiledbgfile,"(%c%c12)", PTS_CODE, HTP_PTS_CODE);
#endif         
      ...
      {
#ifdef PTS_AUTOMATING
        APPL_MESG_INFO(profiledbgfile,"(%c%c13)", PTS_CODE, HTP_PTS_CODE);
#endif         
      ...
      {
#ifdef PTS_AUTOMATING
        APPL_MESG_INFO(profiledbgfile,"(%c%c14)", PTS_CODE, HTP_PTS_CODE);
#endif          
...
void HT_Collector_DataValueRead_CB(uint8_t status, uint16_t data_len, uint8_t *data)
{
  ...
  if (status == BLE_STATUS_SUCCESS) 
  {
    APPL_MESG_INFO(profiledbgfile,"****    Data: ");
      ...
      APPL_MESG_INFO(profiledbgfile,"\n"); 
      APPL_MESG_INFO(profiledbgfile,"(%c%c24)", PTS_CODE, HTP_PTS_CODE);
...
{
#ifdef PTS_AUTOMATING
  APPL_MESG_INFO(profiledbgfile,"(%c%c19)", PTS_CODE, HTP_PTS_CODE);
#endif 
...
{   
#ifdef PTS_AUTOMATING
  APPL_MESG_INFO(profiledbgfile,"(%c%c15)", PTS_CODE, HTP_PTS_CODE);
#endif  
